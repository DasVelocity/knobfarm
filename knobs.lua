RandomStuff:AddCheckbox("AutoKnob", {
    ...
    Callback = function(Value)
        ...
        local function attachToCharacter(character)
            ...
            -- cleanup previous heartbeat if any
            cleanupHeartbeat()
            ...
            -- When humanoid dies we cleanup. CharacterAdded will reattach the script when a new character spawns.
            humanoid.Died:Connect(function()
                cleanupHeartbeat()
                -- NOTE: We do NOT fire any server remotes here to force rejoin.
                -- Reattachment is handled by the CharacterAdded listener below.
            end)
        end

        local function startListening()
            -- ensure we don't attach multiple duplicate listeners
            if charAddedConn then charAddedConn:Disconnect() charAddedConn = nil end

            charAddedConn = player.CharacterAdded:Connect(function(char)
                -- slight delay so the world has time to create things
                task.wait(0.5)
                attachToCharacter(char)
            end)

            -- if player already has a character, attach now
        ...
        if Value then
            startListening()
            notify("AutoKnob enabled (local-only). It will reattach on each respawn in this session.", 3)
        else
            stopAll()
